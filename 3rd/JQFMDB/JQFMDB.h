...
/**
 ...
 @param tableName 表的名称
 @param parameters 每条查找结果放入model(可以是[Book class] or @"Person" or Person实例)或dictionary中
 @param format 条件语句, 如:@"where name = '小李'",
...
//  评论专用
- (NSArray *)jq_comment_lookupTable:(NSString *)tableName fmdb:(FMDatabase *)db dicOrModel:(id)parameters whereFormat:(NSString *)format, ...;
